...
\SynapticSubject{User manual for synaptic version 3.0.0}
...
\texttt{problem}, \texttt{solution}, \texttt{example}, \texttt{exercise},
\texttt{remark}, \texttt{note}, and \texttt{warning} environments share one
...
\texttt{remark}; a custom theorem is boxed automatically. Declaring a name that
already exists is a hard error: the built-in family plus anything declared with
\verb|\SynapticNewTheorem| is the complete environment set.
...
Lecture mode adds teaching-card environments: \texttt{synlearninggoals},
\texttt{synlecturesummary}, and the course metadata commands described above.
...
\begin{itemize}
  ...
  \item \textbf{``Options frozen after loading''} --- \texttt{mode},
    \texttt{lang}, \texttt{fontset}, and typography keys are structural.
...
Version 3.0 removes every backward-compatibility shim. Use the namespaced forms
throughout:
...
{\setlength{\parindent}{0pt}\noindent\begin{tabularx}{\textwidth}{@{}l Y@{}}
  ...
  \midrule
  \texttt{\textbackslash pkg} & \SynapticCmd{SynapticPkg} \\
  \texttt{\textbackslash cmd} & \SynapticCmd{SynapticCmd} \\
  \texttt{\textbackslash acknowledgments} & \SynapticCmd{SynapticAcknowledgments} \\
  \texttt{\textbackslash synapticfrontmatter} & \SynapticCmd{SynapticFrontMatter} \\
  \texttt{\textbackslash synapticmainmatter} & \SynapticCmd{SynapticMainMatter} \\
  \texttt{\textbackslash synapticbackmatter} & \SynapticCmd{SynapticBackMatter} \\
  \texttt{\textbackslash title}/\texttt{\textbackslash author}/\texttt{\textbackslash date} & \SynapticCmd{SynapticTitle} etc. \\
  \texttt{synexample}/\texttt{synremark}/\texttt{synwarning}/\texttt{synexercise}/\texttt{synbox} & \texttt{example}/\texttt{remark}/\texttt{warning}/\texttt{exercise} \\
  \texttt{box-numbering} & \texttt{numbering} \\
  \texttt{title-layout=cover} & \texttt{title-layout=sequence} \\
  \bottomrule
\end{tabularx}}

The statement environments (\texttt{example}, \texttt{exercise}, \texttt{warning}
included) are now first-class members of the built-in family in every mode and
