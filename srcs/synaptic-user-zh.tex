...
\SynapticSubject{synaptic 3.0.0 用户手册}
...
\texttt{remark}、\texttt{note} 与 \texttt{warning} 共享一个计数器。书籍默认按
...
自定义定理也会自动加框。对已有名称重复声明会直接报错：内置定理族加上
\verb|\SynapticNewTheorem| 声明的环境合起来构成完整的环境集合。
...
{\setlength{\parindent}{0pt}\noindent\begin{tabularx}{\textwidth}{@{}l Y@{}}
  ...
  \texttt{\textbackslash title}/\texttt{\textbackslash author}/\texttt{\textbackslash date} & \SynapticCmd{SynapticTitle} 等 \\
...
陈述环境（含 \texttt{example}、\texttt{exercise}、\texttt{warning}）现在在
所有模式中都是内置定理族的一等成员，并共享定理计数器。
