...
\SynapticSubject{synaptic 2.3.0 用户手册}
...
仓库已包含生成好的宏包文件，可将 \texttt{TEXINPUTS} 指向它们：
...
journal 的 \texttt{inline}、book 的 \texttt{cover} 以及 lecture 与 notes 的
\texttt{compact}。\texttt{page} 生成整页标题；\texttt{inline} 强制第一页
内联构成。期刊标题下方使用编辑部风格的双重规则；书籍标题置于短主色条之上；
讲义与笔记信息条使用随主题着色的单一规则。

\subsection{导航}

所有模式在运行页眉下方绘制一条发丝线。发丝线使用独立的装饰性浅色令牌，
使导航保持安静。页码居中于页脚；书籍模式还遵循外侧页眉与双面空白页。
...
教学与笔记卡片共享同一视觉语言：直角、着语义色（主色、次级、警告或成功色）
的左侧色条、浅色表面与可读标题颜色。规则与发丝线由主题主色的低浓度混合
推导，装饰永不与正文争抢。
...
未设置 Synaptic 字段时，\verb|\title|、\verb|\author| 和 \verb|\date| 可被导入。
PDF 字符串只进行一次 Unicode 安全转换。

\section{定理与数学}

内置 \texttt{theorem}、\texttt{lemma}、\texttt{proposition}、
...
\texttt{note} 共享一个计数器。书籍默认按章编号，短文档默认按节编号。定义正
文使用正体；中文命题类正文保持正体，避免伪斜体造成的阅读干扰。

默认（\texttt{theorem-style=boxed}）下，编号的陈述环境以精致的卡片呈现：微
妙的主题色背景、西侧强调条与方角，与教学与笔记卡片语言保持一致。
\texttt{proof} 保持干净、不加框。选择 \texttt{theorem-style=plain} 可恢复无框
经典样式。
...
参数依次为主色、次级色与可选强调色。以下语义令牌始终可用于扩展，并始终
由当前主题推导：
...
讲义模式增加抗冲突的教学环境。每个环境接受可选标题与可选标签：

\begin{itemize}\itemsep0pt
  \item \SynapticPkg{synexample} --- 例题，默认与定理共享计数器
  \item \SynapticPkg{synremark} --- 注记，默认不编号
  \item \SynapticPkg{synwarning} --- 警告，默认不编号
  \item \SynapticPkg{synexercise} --- 练习，默认与定理共享计数器
  \item \SynapticPkg{synbox} --- 通用盒子，可选标题
\end{itemize}

通用 v2.0 别名仅在原名空闲时创建。课程单元还可使用 \texttt{synlearninggoals}
与 \texttt{synlecturesummary}。
...
\begin{itemize}
  ...
  \item \textbf{``mode=book requires a chapter-based class''} --- 请使用
    ...
    的 KOMA 文档类）。
  \item \textbf{``Incomplete ... font bundle''} --- 显式指定的 \texttt{fontset}
    缺少配套字体。请安装完整组合，或改用 \texttt{fontset=auto} 自动回退。
  \item \textbf{选项在加载后冻结} --- \texttt{mode}、\texttt{lang}、
    \texttt{fontset} 等为结构性选项，请在 \verb|\usepackage| 中设定。
  \item \textbf{未找到中文字体} --- 请安装 Noto CJK SC 或 TeX Live 自带的
    Fandol 字体。
  \item \textbf{``Document title is empty''} --- 请在
    \verb|\SynapticMakeTitle| 前设置 \verb|\SynapticTitle|（或 \verb|\title|）。
...
2.x 系列替换了 v1 的选项与命令表面。已移除的 \texttt{style=} 与
\texttt{color=} 选项仍会明确报错并附带迁移提示。
...
v2.3 的公开 API 与 v2.2 一致。小写书籍前后文命令和讲义盒子通用名仍作为有条件的
v2.0 兼容别名保留；新文档应使用带命名空间的公开形式。

\section{开发命令}

\begin{verbatim}
l3build unpack   # 从 synaptic.dtx 重新生成宏包文件
l3build check    # 运行 LuaLaTeX 回归测试（18 组）
