...
\SynapticSubject{synaptic 3.0.0 技术参考}
...
薄封装层把用户选项转发给 \texttt{synaptic-base}，并定义少量文档辅助命令。
...
\noindent{\setlength{\parindent}{0pt}\noindent\begin{tabularx}{\textwidth}{@{}l l Y@{}}
  ...
  \midrule
  \texttt{synaptic.sty} & 封装 & 选项转发、文档辅助命令 \\
...
\texttt{theme}、\texttt{toc} 和 \texttt{toc-layout}。运行时请求中的结构键与未知键
直接报错；宏包选项中的未知键给出警告。
...
\begin{itemize}
  ...
  \item \textbf{只做一次 Unicode 转换}。PDF 元数据以源词元交给 hyperref；
    预先转换会造成双重编码。
  \item \textbf{公开名抗冲突}。陈述环境属于内置定理族；新声明必须
    通过 \verb|\SynapticNewTheorem| 进行，重名即报错。
...
\texttt{claim}、\texttt{property}、\texttt{example}、\texttt{exercise}、
\texttt{criterion}、\texttt{convention}、\texttt{problem}、\texttt{solution}、
\texttt{remark}、\texttt{note} 与 \texttt{warning}）使用模式化共享计数器：
书籍默认按章，其他模式默认按节。在 \texttt{theorem-style=boxed} 之下，编号的陈述
环境在 \texttt{\string\begin{document}} 处由 tcolorbox 包装（主题色背景配西侧强调条）；
\texttt{proof} 保持无框。\texttt{theorem-style=plain} 关闭该包装。例题与练习为
...
新公开名应避免常见名词。陈述环境属于内置定理族：新声明应通过
\verb|\SynapticNewTheorem| 完成，而不是添加别名。
...
3.0 移除了全部向后兼容层：\texttt{\backslash pkg} 与 \texttt{\backslash cmd} 等
别名、小写书籍前后文命令、对标准 \texttt{\backslash title}/\texttt{\backslash author}/\texttt{\backslash date}
的导入、\texttt{box-numbering} 选项与 \texttt{synaptic-boxes} 模块。示例、警告与
练习现在是内置定理族的一等成员；\texttt{title-layout=cover} 更名为
\texttt{title-layout=sequence}；重复声明 \verb|\SynapticNewTheorem| 会直接报错。
新文档应完全使用带命名空间的公开形式。
