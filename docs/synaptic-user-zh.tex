...
\SynapticSubject{synaptic 5.2.0 用户手册}
...
\textbf{beamer} 渲染面向投影的标题帧、语义帧标题、单层语义卡片，以及带页数
进度的克制页脚。它复用课程元数据与讲义教学卡片词汇。显式调用
...
\texttt{theorem}、\texttt{definition}、\texttt{example} 环境保持默认
block 模板。
...
中点时，正文与弱化文字自动切换为浅色墨，发丝线与边框由加深改为提亮，链接
使用主色的浅色变体。Beamer 模式的帧背景画布遵循同一设置，深色演示无需
额外配置。以 \texttt{background=white} 恢复默认。
...
提供克制的页脚锚点。
Beamer 在帧标题下方复用同一发丝线，并在页脚底部绘制安全钳制的进度线；首轮编译尚未
得知总帧数时也不会溢出。
