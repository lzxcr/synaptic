...
\SynapticSubject{synaptic 5.2.0 技术参考}
...
并派生表面、边框、分隔线、链接、可读弱化文字、装饰浅色与正文颜色。页面背景键
\texttt{background}（任意 xcolor 表达式）成为 \texttt{syn-bg} 基础令牌；所有
派生角色与它混合，并按相对亮度切换：背景偏暗时正文与弱化文字翻转成浅色墨，
装饰线由加深改为提亮。print 与 mono 输出模式只改变派生角色，不改变组件代码。
危险、警告和成功色是稳定的语义常量，不随品牌主色改变含义。
...
此分支执行。模式模块安装语义 Beamer 颜色、可变高帧标题、原生卡片、列表标记、安全钳制的
进度页脚，以及显式的目录与章节过渡帧命令。
