...
\SynapticSubject{User manual for synaptic version 5.3.0}
...
semester, lecture number) and wraps theorems and examples in a restrained card
...
\textbf{beamer} renders a projection-first title frame, signal-rail frame
titles, numbered agenda entries, single-layer semantic cards, and a restrained
footer with tracked frame progress. It
...
standard block semantics, rendered by Synaptic's light native-block card with
an inline title and semantic west rail.
...
stays coherent without touching the theme. When relative luminance falls below
the mid-point, neutral text flips to light ink and the primary, secondary,
accent, danger, warning, and success roles lift to remain legible. Registered
theme colours stay stable, so returning to a light background is exactly
reversible. In Beamer mode the frame canvas follows the same setting; dark
presentations need no separate theme machinery. Restore the default with
\texttt{background=white}.
...
Beamer extends the motif beneath each frame title: a short primary signal and
accent node lead into a quiet hairline across the usable measure. Its footer
places a clamped active progress segment over a faint track inside the text
margins, so the first compilation is safe before the final frame count is
known.
...
Theorem, title-metadata, teaching, notes, and Beamer cards share one base visual
primitive: a very small corner radius, a semantic west rail where needed, a
faint background and north hairline, modest internal padding, and readable
title colouring.
...
{\setlength{\parindent}{0pt}\noindent\begin{tabularx}{\textwidth}{@{}l l Y@{}}
  ...
  \texttt{syn-surface} & middle surface & default cards and panels \\
...
Adjacent \texttt{syn-surface-soft} and \texttt{syn-surface-strong} tokens
complete the surface ladder. \texttt{syn-on-primary} and
\texttt{syn-on-danger} provide inverse ink for solid semantic fills.
