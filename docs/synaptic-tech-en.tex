...
\SynapticSubject{Technical reference for synaptic version 5.3.0}
...
\noindent{\setlength{\parindent}{0pt}\noindent\begin{tabularx}{\textwidth}{@{}l l Y@{}}
  ...
  \texttt{synaptic-title.sty} & core & metadata model and five title renderers \\
  ...
  \texttt{synaptic-beamer.sty} & mode & slide templates, progress, agenda/dividers \\
  \bottomrule
\end{tabularx}}
...
Structural or unknown keys in a runtime request are errors; unknown package
options are errors too.

Book mode is rejected unless both chapter and front-matter interfaces exist.
The class contract is checked after option parsing: Beamer mode requires the
Beamer class, while the four print modes require KOMA-Script. LuaLaTeX and
class requirements are fatal diagnostics rather than delayed
...
Built-in themes are stored in a global property list. Activation copies their
primary, secondary, and accent colours into stable base roles, then derives the
public display colours, a three-step surface ladder, borders, rules, links,
readable-muted, decorative-muted, and text tokens. The \texttt{background} key
(any xcolor expression) becomes the \texttt{syn-bg} base token. Every derived
role blends against it; a relative-luminance switch flips neutral text and
lifts brand, accent, and status roles when the canvas is dark. Because the base
roles are not overwritten, a later background change is reversible. Print and
mono delivery modes alter the same derivation without changing component code.
Danger, warning, and success retain stable semantic bases independent of the
brand palette.
...
Beamer takes a separate branch: the class retains ownership of the slide
canvas, while \texttt{measure} maps to symmetric frame margins and
\texttt{density} maps to presentation leading. KOMA-only geometry, caption,
list, heading, and TOC declarations are never executed in this branch. The
mode module installs semantic Beamer colours, a variable-height signal-rail
frame title, aligned numbered agenda entries, single-layer native blocks, list
markers, a tracked and clamped progress footer, and explicit agenda and
section-divider frame commands. The theorem module supplies one shared card
primitive that title metadata and every mode-specific card extend.
...
repeatable. A common metadata model feeds five renderers: inline journal,
book title sequence, course masthead, compact notes masthead, and a Beamer
title frame. The title
...
In Beamer mode, the namespaced setters synchronize with the class-native title,
author, subject, and keyword interfaces before \verb|\begin{document}|; the
shared late metadata writer is skipped because Beamer freezes those fields at
document start.
...
\begin{itemize}
  ...
  \item Add modes in the base key family, class validation, layout dispatch,
    title/TOC dispatch, and mode-module loader. State and test the required
    class capability.
...
{\setlength{\parindent}{0pt}\noindent\begin{tabularx}{\textwidth}{@{}l Y@{}}
  ...
  \midrule
  \texttt{journal}, \texttt{lecture}, \texttt{notes}, \texttt{book},
  \texttt{beamer} & core
