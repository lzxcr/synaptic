...
\SynapticSubject{Technical reference for synaptic version 5.2.0}
...
The \texttt{background} key (any xcolor expression) becomes the \texttt{syn-bg}
base token; every derived role blends against it, and a relative-luminance
switch flips text to light inks and lightens decorative rules when the
background is dark. Print and mono delivery modes alter derived roles without
changing component code. Danger, warning, and success
...
mode module installs semantic Beamer colours, a variable-height frame-title
template, native blocks, list markers, a clamped progress footer, and explicit
agenda and section-divider frame commands.
